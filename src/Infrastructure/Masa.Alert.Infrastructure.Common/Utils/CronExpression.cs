...
{
    protected const string I18nPrefix = "$masaBlazor.cron.exception";
        ...
        try
        {
            ...
            foreach (string exprTok in exprsTok)
            {
                ...
                if (exprOn == DayOfMonth && expr.IndexOf('L') != -1 && expr.Length > 1 && expr.IndexOf(",", StringComparison.Ordinal) >= 0)
                {
                    ...
                    throw new FormatException($"{I18nPrefix}.LNotSupportMonth");
                }
                // throw an exception if L is used with other days of the week
                if (exprOn == DayOfWeek && expr.IndexOf('L') != -1 && expr.Length > 1 && expr.IndexOf(",", StringComparison.Ordinal) >= 0)
                {
                    throw new FormatException($"{I18nPrefix}.LNotSupportWeek");
                }
                if (exprOn == DayOfWeek && expr.IndexOf('#') != -1 && expr.IndexOf('#', expr.IndexOf('#') + 1) != -1)
                {
                    throw new FormatException($"{I18nPrefix}.NotSupportMutilSpecifynthWeekDay");
                }
            ...
            {
                throw new FormatException($"{I18nPrefix}.CannotSpecifyDayAndWeek");
            }
        }
        catch (FormatException)
        {
    ...
    protected virtual int StoreExpressionVals(int pos, string s, int type)
    {
        ...
        if (c >= 'A' && c <= 'Z' && !s.Equals("L") && !s.Equals("LW") && !regex.IsMatch(s))
        {
            ...
            if (type == Month)
            {
                ...
                if (sval <= 0)
                {
                    throw new FormatException(string.Format($"{I18nPrefix}.InvalidMonthValue", sub));
                }
                if (s.Length > i + 3)
                {
                    c = s[i + 3];
            ...
            else if (type == DayOfWeek)
            {
                ...
                if (sval < 0)
                {
                    throw new FormatException(string.Format($"{I18nPrefix}.InvalidWeekdayValue", sub));
                }
                if (s.Length > i + 3)
                {
                    c = s[i + 3];
                    ...
                    else if (c == '#')
                    {
                        ...
                        {
                            throw new FormatException($"{I18nPrefix}.InvalidnthDayOfWeek");
                        }
                    }
                    else if (c == '/')
                    {
                        ...
                        {
                            throw new FormatException($"{I18nPrefix}.InvalidEveryDayOfWeek");
                        }
                    }
                    else if (c == 'L')
                    {
                    ...
                    {
                        throw new FormatException(string.Format($"{I18nPrefix}.IllegalCharactersPosition", sub));
                    }
                }
            }
            else
        ...
        if (c == '?')
        {
            ...
            {
                throw new FormatException(string.Format($"{I18nPrefix}.IllegalCharactersAfer?", s[i]));
            }
            if (type != DayOfWeek && type != DayOfMonth)
            {
                throw new FormatException($"{I18nPrefix}.Characters?OnlySupport");
            }
        ...
        if (startsWithAsterisk || c == '/')
        {
            ...
            {
                throw new FormatException($"{I18nPrefix}.MustFollowByInteger");
            }
    ...
    private static void CheckIncrementRange(int incr, int type)
    {
        ...
        {
            throw new FormatException(string.Format($"{I18nPrefix}.IncrementCannotLargeThen", 60, incr));
        }
        if (incr > 23 && type == Hour)
        {
            throw new FormatException(string.Format($"{I18nPrefix}.IncrementCannotLargeThen", 24, incr));
        }
        if (incr > 31 && type == DayOfMonth)
        {
            throw new FormatException(string.Format($"{I18nPrefix}.IncrementCannotLargeThen", 31, incr));
        }
        if (incr > 7 && type == DayOfWeek)
        {
            throw new FormatException(string.Format($"{I18nPrefix}.IncrementCannotLargeThen", 7, incr));
        }
        if (incr > 12 && type == Month)
        {
            throw new FormatException(string.Format($"{I18nPrefix}.IncrementCannotLargeThen", 12, incr));
        }
    }
    ...
    protected virtual int CheckNext(int pos, string s, int val, int type)
    {
        ...
        if (c == 'L')
        {
            if (type == DayOfWeek)
            {
                if (val < 1 || val > 7)
                {
                    throw new FormatException($"{I18nPrefix}.InvalidWeekday");
                }
                lastdayOfWeek = true;
            }
            else
            {
                throw new FormatException(string.Format($"{I18nPrefix}.OptionNotValid", "'L'", i));
            }
        ...
        if (c == 'W')
        {
            ...
            {
                throw new FormatException(string.Format($"{I18nPrefix}.OptionNotValid", "'W'", i));
            }
            if (val > 31)
            {
                throw new FormatException($"{I18nPrefix}.WOptionLargeThen31");
            }
        ...
        if (c == '#')
        {
            if (type != DayOfWeek)
            {
                throw new FormatException(string.Format($"{I18nPrefix}.OptionNotValid", "'#'", i));
            }
            i++;
            try
            {
                ...
                if (nthdayOfWeek < 1 || nthdayOfWeek > 5)
                {
                    throw new Exception($"{I18nPrefix}.InvalidNthOfWeek");
                }
            }
            catch (Exception)
            {
        ...
        if (c == 'C')
        {
            ...
            {
                throw new FormatException(string.Format($"{I18nPrefix}.OptionNotValid", "'C'", i));
            }
            ISet<int> data = GetSet(type);
            data.Add(val);
            i++;
        ...
        if (c == '/')
        {
            ...
            throw new FormatException(string.Format($"{I18nPrefix}.IllegalCharactersAfer/", c));
        }
    ...
    protected virtual void AddToSet(int val, int end, int incr, int type)
    {
        ...
        if (type == Second || type == Minute)
        {
            ...
            {
                throw new FormatException($"{I18nPrefix}.InvalidMinuteAndSecond");
            }
        }
        else if (type == Hour)
        {
            ...
            {
                throw new FormatException($"{I18nPrefix}.InvalidHour");
            }
        }
        else if (type == DayOfMonth)
        {
            if ((val < 1 || val > 31 || end > 31) && val != AllSpecInt
                && val != NoSpecInt)
            {
                throw new FormatException($"{I18nPrefix}.InvalidDay");
            }
        }
        else if (type == Month)
        {
            ...
            {
                throw new FormatException($"{I18nPrefix}.InvalidMonth");
            }
        }
        else if (type == DayOfWeek)
        {
        ...
        if (stopAt < startAt)
        {
            switch (type)
            {
                ...
                case DayOfMonth:
                    max = 31;
                    break;
                case Year: throw new ArgumentException($"{I18nPrefix}.InvalidStartYear");
    ...
    public virtual DateTimeOffset? GetTimeAfter(DateTimeOffset afterTimeUtc)
    {
        ...
        while (!gotOne)
        {
            ...
            else
            {
                ...
                throw new Exception($"{I18nPrefix}.CannotSpecifyDayAndWeek");
            }
